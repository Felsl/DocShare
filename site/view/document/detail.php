...
<div class="row">
    <div class="col-md-8">
        ...
        <p class="text-light">Danh mục: <?= htmlspecialchars($category->getName() ?? 'N/A') ?> • Người upload:
            <?= htmlspecialchars($uploader->getName() ?? 'N/A') ?>
        </p>
        <p><?= nl2br(htmlspecialchars($doc->getDescription())) ?></p>

        <div class="mb-3">
            <a class="btn btn-primary" href="<?= $base ?>/<?= htmlspecialchars($doc->getFilename()) ?>" download>⬇ Tải về</a>
        ...
        <?php if (empty($comments)): ?>
            <p class="text-light">Chưa có bình luận.</p>
        <?php else:
            foreach ($comments as $c): ?>
                <div class="border rounded p-2 mb-2">
                    <div class="small text-light"><?= htmlspecialchars($c->getCreatedAt()) ?></div>
                    <div><?= nl2br(htmlspecialchars($c->getContent())) ?></div>
                </div>
            <?php endforeach; endif; ?>

        <?php if (isset($_SESSION['user_id'])): ?>
            <form method="post" action="index.php?c=comment&a=store">
        ...
        <?php else: ?>
  					<a href="index.php?c=user&a=login">Đăng nhập</a> để bình luận.
        <?php endif; ?>
    </div>
